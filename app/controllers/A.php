...
class A extends CI_Controller
{
	...
	function view_account($id)
	{
		if ($this->admin->is_logged()) {
			...
			if ($this->input->get('login')) {
			...
			} elseif ($this->input->get('builder') && $this->input->get('domain')) {
				...
				$res = $this->account->get_account($id);
				if ($res !== false) {
					if ($res['account_status'] === 'suspended' || $res['account_status'] === 'deactivated') {
	...
	function account_settings($id)
	{
		if ($this->admin->is_logged()) {
			...
			if ($this->input->post('update_label')) {
				...
				$res = $this->account->get_account($id);
				if ($res !== false) {
					if (strlen($this->input->post('password')) > 4 && strlen($this->input->post('old_password')) > 4) {
